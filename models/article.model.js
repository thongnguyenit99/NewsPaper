...
module.exports = {
    ...
    bestnew1: function () {
        return db.load(`select a.c_ID ,a.id,a.title,a.abstract,a.author,DATEDIFF(CURDATE(), a.public_date) AS day,a.featured,a.views,a.isActive,a.images,c.ID,c.tc_ID,c.c_Name,c.c_Large 
    ...
    ArtSameCat: function (id) {
        return db.load(`SELECT * FROM ${TBL_article} a join categories c on a.c_ID=c.ID
            WHERE a.c_ID=${id} and a.isActive=1 ORDER BY rand() LIMIT 5`)
    },
